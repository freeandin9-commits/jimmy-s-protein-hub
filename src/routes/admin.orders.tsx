...
function OrdersPage() {
  ...
  const clearHistory = async () => {
    // എല്ലാ ഓർഡറുകളും ഡിലീറ്റ് ചെയ്യുന്നു
    const { error } = await supabase.from("orders").delete().neq("id", "none");
    if (error) {
  ...
  return (
    <div className="space-y-8 p-6 bg-zinc-950 text-zinc-50 min-h-screen">
            ...
            <AlertDialogContent className="bg-zinc-900 border-zinc-800 text-zinc-100">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription className="text-zinc-400">
                  This action will permanently delete all order records. This cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel className="bg-zinc-800 text-white">Cancel</AlertDialogCancel>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-zinc-800">
            {isLoading ? (
              ...
              (data ?? []).map((o: any) => (
                <tr key={o.id} className="hover:bg-zinc-800/50 cursor-pointer" onClick={() => setSelected(o)}>
                  <td className="p-4 font-mono text-amber-400">#{String(o.id).slice(0, 8)}</td>
                  <td className="p-4">{o.customer_name}</td>
                  <td className="p-4">{formatPrice(o.total, o.currency)}</td>
                  <td className="p-4 uppercase text-[10px] font-bold">{o.status}</td>
                </tr>
              ))
            )}
          </tbody>
      ...
      {/* (Sheet component logic remains same as before) */}
    </div>
  );
}
