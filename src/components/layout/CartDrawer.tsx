...
// Custom Order ID ജനറേറ്റർ ഫങ്ഷൻ (Format: NSDDMMYYYY12345)
function generateCustomOrderId(): string {
  const now = new Date();
  const day = String(now.getDate()).padStart(2, "0");
  const month = String(now.getMonth() + 1).padStart(2, "0");
  const year = now.getFullYear();
  const randomNumber = Math.floor(10000 + Math.random() * 90000); // 5 Digit unique code
...
export function CartDrawer() {
  ...
  const handleConfirm = async () => {
    ...
    // ടൈപ്പ് എറർ ഒഴിവാക്കാൻ ഒറിജിനൽ വേരിയബിളുകൾ മാത്രം പാസ്സ് ചെയ്യുന്നു
    const result = await logOrderToDatabase([...items], customer);
    setSubmitting(false);
    ...
    // സേവിങ് വിജയകരമായാൽ കസ്റ്റം ലേബൽ ഐഡി നിർബന്ധമായി ഇവിടെ ലോക്കൽ സ്റ്റേറ്റിൽ സെറ്റ് ചെയ്യുന്നു
    const customOrderId = generateCustomOrderId();
    setConfirmedRef(customOrderId);
    setConfirmed(true);
    toast.success(`Order ${customOrderId} confirmed!`);
  };
  ...
  const finalizeAndSend = async (mode: "whatsapp" | "copy") => {
    ...
    if (!orderRef) {
      ...
      if (result && result.orderRef) {
        orderRef = generateCustomOrderId();
      }
    }
