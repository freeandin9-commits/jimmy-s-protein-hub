...
export function Header() {
  ...
  // കാർട്ട് ഓപ്പൺ ചെയ്യാനുള്ള ഫങ്ക്ഷൻ സ്റ്റോറിൽ നിന്ന് കൃത്യമായി എടുക്കുന്നു.
  // സ്റ്റോറിൽ 'open' അല്ലെങ്കിൽ 'setOpen' ഏതാണോ ഉള്ളത് അതിനനുസരിച്ച് ഇത് വർക്ക് ചെയ്യും.
  const cartStore = useCartStore() as any;
  const handleCartOpen = () => {
    if (typeof cartStore.open === "function") {
      cartStore.open();
    } else if (typeof cartStore.setOpen === "function") {
  ...
  return (
    // sticky top-0, z-50, shadow-md നൽകി നാവിഗേഷൻ ബാർ മുകളിൽ ലോക്ക് ചെയ്തു
    <header className="sticky top-0 z-50 w-full bg-white/95 text-black border-b border-slate-200 shadow-md backdrop-blur-md transition-all">
      ...
      {/* Mobile Drawer Menu - സ്ക്രോൾ ചെയ്യുമ്പോഴും ഇതിന്റെ പൊസിഷൻ കൃത്യമായിരിക്കും */}
      {mobileOpen && (
